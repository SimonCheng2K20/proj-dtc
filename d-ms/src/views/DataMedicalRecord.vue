        ...
        <el-col :md="18">
          <CardBox v-if="formEditCopy.Id">
            <div class="dataMedicalRecordList">
              <div class="dataMedicalRecordLabel mr-1">病歷夾：</div>
            ...
            <el-form ref="postform" :model="formEditCopy" :rules="rules" class="mb-1" label-position="left">
              <InputLabel class="mb-1" title="病歷夾名稱" prop="Name">
...
export default {
  ...
  methods: {
    async getData() {
      this.$store.commit('loading',true)
      this.list=await this.$api.getFolder().then(res=>res.Items)
      this.$store.commit('medicalRecordList',this.$global.treeToArr(this.$global.arrToTree(this.list)))
      this.$store.commit('loading',false)
    },
    handleNodeClick(e) {
      //console.log(e)
    ...
    async control(type,x) {
      ...
      if(type==='add1') {
        this.formEditCopy.Name+=' - 同層病歷夾'
        this.formEditCopy.Notes=null
      }
      else if(type==='add2') {
        this.formEditCopy.Name+=' - 子病歷夾'
        this.formEditCopy.Notes=null
    ...
    async del() {
      if(!confirm(`確定刪除${this.formEditCopy.Name}?`)) return null
      this.$store.commit('loading',true)
      await this.$api.delFolder(this.formEditCopy.Id)
      await this.getData()
      this.formEditCopy=JSON.parse(JSON.stringify(this.form))
    ...
    async sub() {
      this.$refs.postform.validate(async (valid) => {
        if(valid) {
          this.$store.commit('loading',true)
          if(this.type==='edit') {
            await this.$api.putFolder(this.formEditCopy)
            this.$message.success('修改成功')
          }
          else {
            await this.$api.postFolder(this.formEditCopy)
