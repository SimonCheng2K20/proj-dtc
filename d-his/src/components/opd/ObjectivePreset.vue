<template>
  <q-dialog ref="dialogRef" full-width full-height no-backdrop-dismiss>
    <my-dialog-content header="O 客觀檢查組套清單" width="1280px" @close="onDialogCancel">
      <q-table
        v-bind="QTableProps"
        :columns="columns"
        :rows="rows"
      ...
      >
        ...
        <template v-slot:bottom>
          <my-pagination
            class="mx-auto p-4"
            v-model="pagination"
            v-on:update:model-value="GetPresetObjective()"
          ></my-pagination>
        </template>
      </q-table>
    </my-dialog-content>
...
import { getPresetObjective } from 'api'
...
const rows = ref([])
const GetPresetObjective = async () => {
