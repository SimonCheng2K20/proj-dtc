<template>
  <q-dialog ref="dialogRef" full-width full-height no-backdrop-dismiss>
    <my-dialog-content header="M 用藥組套清單" width="1280px" @close="onDialogCancel">
      <q-table
        v-bind="QTableProps"
        :columns="columns"
        :rows="rows"
      ...
      >
        ...
        <template v-slot:bottom>
          <my-pagination
            class="mx-auto p-4"
            v-model="pagination"
            v-on:update:model-value="GetPresetMedication()"
          ></my-pagination>
        </template>
      </q-table>
    </my-dialog-content>
...
import { getPresetMedication, getIpdMedicationPresets } from 'api'
...
const rows = ref([])
const GetPresetMedication = async () => {
  ...
  try {
    ...
    const res =
      props.presetKeyName === 'Medication' ? await getPresetMedication(query) : await getIpdMedicationPresets(query)
    if (res.status === 200) {
...
const toPaste = (row) => {
  if (props.presetKeyName === 'Medication') {
    emit('paste', row.PresetMedicationItems)
  } else {
    emit('paste', row.IpdMedicationPresetItems)
  }
}
