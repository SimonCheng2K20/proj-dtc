<template>
  <q-dialog ref="dialogRef" full-width full-height no-backdrop-dismiss>
    <my-dialog-content header="S 主訴組套清單" width="1280px" @close="onDialogCancel">
      <q-table v-bind="QTableProps" :columns="columns" :rows="rows" row-key="Id" :pagination="pagination" :loading="tableLoading">
        ...
        <template v-slot:bottom>
          <my-pagination class="mx-auto p-4" v-model="pagination" v-on:update:model-value="GetPresetSubjective()"></my-pagination>
        </template>
      </q-table>
    </my-dialog-content>
  </q-dialog>
...
import { getPresetSubjective } from 'api'
import SubjectivePresetEditItem from './SubjectivePresetEditItem.vue'
...
const rows = ref([])
const GetPresetSubjective = async () => {
...
const toEdit = (row) => {
  const dialog = $q
    .dialog({
      component: SubjectivePresetEditItem,
      componentProps: {
        editType: 'edit',
        id: row.Id,
      },
