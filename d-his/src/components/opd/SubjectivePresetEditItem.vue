<template>
  <q-dialog ref="dialogRef" full-width no-backdrop-dismiss>
    <my-dialog-content :header="editHeader" @close="onDialogCancel" :loading="getDataloading">
      <div v-if="editType === 'delete'" class="text-negative mb-1">是否確定刪除此主訴組套？</div>
...
import { getPresetSubjectiveItem, postPresetSubjectiveItem, putPresetSubjectiveItem, deletePresetSubjectiveItem } from 'api'
...
const GetPresetSubjectiveItem = async (id) => {
  try {
    getDataloading.value = true
    const res = await getPresetSubjectiveItem(id)
...
const PostPresetItem = async (editItem) => {
  try {
    ...
    const res = await postPresetSubjectiveItem(editItem)
    $q.notify({
      type: 'positive',
      message: `新增主訴組套成功`,
      position: 'top',
    ...
    onDialogOK()
    presetStore.getPreset('Subjective')
  } catch (err) {
    console.log(err)
    $q.notify({
      type: 'negative',
      message: `新增主訴組套失敗：` + (typeof err.data === 'string' ? err.data : JSON.stringify(err.data)),
      position: 'top',
    })
  } finally {
...
const PutPresetItem = async (editItem) => {
  try {
    editLoading.value = true
    const res = await putPresetSubjectiveItem(editItem)
    $q.notify({
      type: 'positive',
      message: `儲存主訴組套成功`,
      position: 'top',
    ...
    $q.notify({
      type: 'negative',
      message: `儲存主訴組套失敗：` + (typeof err.data === 'string' ? err.data : JSON.stringify(err.data)),
      position: 'top',
    })
  } finally {
    editLoading.value = false
...
const DeletePresetItem = async (id) => {
  try {
    editLoading.value = true
    const res = await deletePresetSubjectiveItem(id)
    $q.notify({
      type: 'positive',
      message: `刪除主訴組套成功`,
      position: 'top',
    ...
    $q.notify({
      type: 'negative',
      message: `刪除主訴組套失敗：` + (typeof err.data === 'string' ? err.data : JSON.stringify(err.data)),
      position: 'top',
    })
  } finally {
    editLoading.value = false
...
const init = async () => {
  if (props.id) GetPresetSubjectiveItem(props.id)
}
init()
</script>
