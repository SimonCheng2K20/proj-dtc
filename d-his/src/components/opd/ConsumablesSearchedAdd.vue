<template>
  <q-dialog ref="dialogRef" no-backdrop-dismiss full-width>
    <my-dialog-content header="新增手術耗材" @close="onDialogCancel" :loading="getDataLoading">
      <div class="py-3 px-5 font-bold min-h-[600px]">
        <div
          class="inline-block mb-2 cursor-pointer"
          v-for="(item, index) in guideFlow.data"
          :key="index"
          @click="guideFlowSelect(index)"
        >
          <span class="inline-block border border-blue-300 text-white p-1" :class="levelColor(index)">{{
            item.Name
          }}</span>
          <q-icon color="primary" name="arrow_right" size="md" v-if="index < 1"></q-icon>
        </div>
        <hr />
        <div
          class="inline-block mt-2"
          :class="getDataLoading ? 'cursor-not-allowed' : 'cursor-pointer'"
          v-for="(item, index) in quesFlow.data"
          :key="index"
          @click="quesFlowSelect(index)"
        >
          <span
            :class="guideSelected === index ? 'text-blue-400' : 'text-blue-800'"
            class="inline-block hoverColor"
            v-if="index < guideFlow.data.length + 1"
          >
            {{ item }}
          </span>
          <q-icon
            color="primary"
            name="arrow_right"
            size="sm"
            v-if="item !== '查詢結果' && index < guideFlow.data.length + 1"
          ></q-icon>
        </div>
        <div
          class="my-3 grid grid-cols-6 gap-3"
          :style="`grid-template-rows: repeat(${Math.ceil(labels?.data.length / 6)}, minmax(0, 1fr))`"
          v-if="guideSelected !== 2"
        >
          <div
            class="labels-div text-center grid grid-cols-1 items-center p-1 text-xl"
            :class="levelColor(guideSelected)"
            v-for="(item, index) in labels.data"
            :key="index"
            @click="labelsSelect(item)"
          >
            {{ item.Name }}
          </div>
        </div>
        <div v-else>
          <q-table
            ref="tableRef"
            v-bind="QTableProps"
            :rows="tableData"
            :columns="columns"
            row-key="HospitalCode"
            :pagination="pagination"
            wrap-cells
          >
            <template #body-cell-action="{ col, row }">
              <q-td :props="{ col }">
                <q-btn color="primary" @click="onAdd(row)" label="帶入" unelevated dense></q-btn>
              </q-td>
            </template>

            <template v-slot:bottom>
              <my-pagination
                class="mx-auto p-4"
                v-model="pagination"
                v-on:update:model-value="GetSpecialMedicalMaterial(guideFlow.data[1].No)"
              ></my-pagination>
            </template>
          </q-table>
        </div>

        <div v-if="labels.data.length === 0 && !getDataLoading">暫無資料</div>
      </div>
    </my-dialog-content>
  </q-dialog>
</template>

<script setup>
import { ref, reactive, computed, watch, watchEffect } from 'vue'
import { QTableProps, QSelectProps, QInputProps } from 'utils/quasar-extends/base-props.js'
import { useDialogPluginComponent, useQuasar } from 'quasar'
import { getSubCodeList, getSpecialCategoryList, getSpecialMedicalMaterial } from 'api'
import setSearchQuery from 'utils/setSearchQuery.js'

const { dialogRef, onDialogOK, onDialogCancel } = useDialogPluginComponent()

const props = defineProps({
  pcsTableData: Array,
})

const getDataLoading = ref(false)
const guideFlow = reactive({ data: [] })
const guideSelected = ref(0)

const quesFlow = reactive({ data: ['請點選大小類碼名稱', '功能類別名稱', '查詢結果'] })

const labels = reactive({
  data: [],
})
const columns = [
  {
    name: 'HospitalCode',
    label: '院內代碼',
    field: 'HospitalCode',
    align: 'center',
    headerClasses: 'sticky top-0 z-10 w-[200px]',
  },
  {
    name: 'SpecialCode',
    label: '特殊代碼',
    field: 'SpecialCode',
    align: 'center',
    headerClasses: 'sticky top-0 z-10 w-[200px]',
  },
  { name: 'Cname', label: '中文名稱', field: 'Cname', align: 'center', headerClasses: 'sticky top-0 z-10 ' },
  {
    name: 'Category',
    label: '功能類別',
    field: 'Category',
    align: 'center',
    headerClasses: 'sticky top-0 z-10 w-[120px]',
  },
  { name: 'Unit', label: '單位', field: 'Unit', align: 'center', headerClasses: 'sticky top-0 z-10 w-[68px]' },
  { name: 'PayPoint', label: '單價', field: 'PayPoint', align: 'center', headerClasses: 'sticky top-0 z-10 w-[68px]' },
  { name: 'action', label: '操作', field: 'action', align: 'center', headerClasses: 'sticky top-0 z-10 w-[68px]' },
]

const pagination = ref({
  page: 1,
  rowsPerPage: 10,
  rowsNumber: 0,
})

const levelColor = (level) => {
  switch (level) {
    case 0: {
      return 'color-bg-purple'
    }
    case 1: {
      return 'color-bg-green'
    }
    case 2: {
      return 'color-bg-lightblue'
    }
    case 3: {
      return 'color-bg-pink'
    }
    case 4: {
      return 'color-bg-orange'
    }
    case 5: {
      return 'color-bg-brown'
    }
    case 6: {
      return 'color-bg-blue'
    }
    case 7: {
      return 'color-bg-purple'
    }
    case 8: {
      return 'color-bg-green'
    }
    case 9: {
      return 'color-bg-lightblue'
    }
    default: {
      return 'color-bg-purple'
    }
  }
}

const guideFlowSelect = (index) => {
  guideSelected.value = index + 1
  if (index === 0) {
    GetSpecialCategoryList(guideFlow.data[0].No)
  } else if (index === 1) {
    GetSpecialMedicalMaterial(guideFlow.data[1].No)
  }
}
const quesFlowSelect = (index) => {
  guideSelected.value = index
  if (index === 0) {
    GetSubCodeList()
  } else if (index === 1) {
    GetSpecialCategoryList(guideFlow.data[0].No)
  } else if (index === 2) {
    GetSpecialMedicalMaterial(guideFlow.data[1].No)
  }
}

const labelsSelect = (item) => {
  pagination.value.page = 1
  guideFlow.data = guideFlow.data.slice(0, guideSelected.value)
  guideSelected.value++
  guideFlow.data = guideFlow.data.concat([item])
  if (guideSelected.value === 1) GetSpecialCategoryList(item.No)
  else GetSpecialMedicalMaterial(item.No)
}

const GetSubCodeList = async () => {
  labels.data = []
  getDataLoading.value = true
  try {
    const res = await getSubCodeList()
    if (res.status === 200) {
      labels.data = res.data
    }
  } catch (error) {
    console.log(error)
  } finally {
    getDataLoading.value = false
  }
}

const GetSpecialCategoryList = async (itemNo) => {
  labels.data = []
  getDataLoading.value = true
  try {
    const res = await getSpecialCategoryList()
    if (res.status === 200) {
      labels.data = res.data.filter((item) => {
        return item.No.includes(itemNo)
      })
    }
  } catch (error) {
    console.log(error)
  } finally {
    getDataLoading.value = false
  }
}
const tableData = ref([])
const GetSpecialMedicalMaterial = async (itemNo) => {
  getDataLoading.value = true
  try {
    const query = setSearchQuery({
      $top: pagination.value.rowsPerPage,
      $skip: (pagination.value.page - 1) * pagination.value.rowsPerPage,
      $filter: `Category eq '${itemNo}'`,
    })
    const res = await getSpecialMedicalMaterial(query)
    if (res.status === 200) {
      pagination.value.rowsNumber = res.data.Count
      tableData.value = res.data.Items
    }
  } catch (error) {
    console.log(error)
  } finally {
    getDataLoading.value = false
  }
}

const onAdd = (row) => {
  onDialogOK(row)
}

const init = () => {
  GetSubCodeList()
}
init()
</script>

<style lang="scss" scoped>
.hoverColor {
  &:hover {
    color: #0696ac;
  }
}

.labels-div {
  border: 1px solid #1d67dc;
  border-radius: 5px;
  color: white;
  cursor: pointer;
}

.color-bg-purple {
  background-color: #a05ef8;
  &:hover {
    background-color: #560fb4;
  }
}
.color-bg-green {
  background-color: #56b849;
  &:hover {
    background-color: #167e09;
  }
}
.color-bg-lightblue {
  background-color: #11bbd6;
  &:hover {
    background-color: #0696ac;
  }
}
.color-bg-pink {
  background-color: #e93592;
  &:hover {
    background-color: #950f54;
  }
}
.color-bg-orange {
  background-color: #f68132;
  &:hover {
    background-color: #c7601b;
  }
}
.color-bg-brown {
  background-color: #745145;
  &:hover {
    background-color: #442013;
  }
}
.color-bg-blue {
  background-color: #1868e1;
  &:hover {
    background-color: #063d91;
  }
}
</style>

