<template>
  <q-dialog ref="dialogRef" no-backdrop-dismiss full-width>
    <my-dialog-content header="新增治療處置代碼" @close="onDialogCancel" :loading="getDataLoading">
      <div class="py-3 px-5 font-bold min-h-[600px]">
        <div
          class="inline-block mb-2 cursor-pointer"
          v-for="(item, index) in guideFlow.data"
          :key="index"
          @click="guideFlowSelect(index)"
        >
          <span class="inline-block border border-blue-300 text-white p-1" :class="levelColor(index)"
            >{{ item.EName }}{{ item.Name }}</span
          >
          <q-icon color="primary" name="arrow_right" size="sm" v-if="index < guideFlow.data.length - 1"></q-icon>
        </div>
        <hr />
        <div
          class="inline-block mt-2"
          :class="getDataLoading ? 'cursor-not-allowed' : 'cursor-pointer'"
          v-for="(item, index) in quesFlow.data"
          :key="index"
          @click="quesFlowSelect(index, item)"
        >
          <span
            :class="guideSelected === index ? 'text-blue-400' : 'text-blue-800'"
            class="inline-block hoverColor"
            v-if="index < guideFlow.data.length + 1"
          >
            {{ item }}
          </span>
          <q-icon
            color="primary"
            name="arrow_right"
            size="sm"
            v-if="item !== '查詢結果' && index < guideFlow.data.length + 1"
          ></q-icon>
        </div>
        <div
          class="my-3 grid grid-cols-6 gap-3"
          :style="`grid-template-rows: repeat(${Math.ceil(labels?.data.length / 6)}, minmax(0, 1fr))`"
          v-if="!showTable"
        >
          <div
            class="labels-div text-center grid grid-cols-1 items-center p-1 text-xl"
            :class="levelColor(guideSelected)"
            v-for="(item, index) in labels.data"
            :key="index"
            @click="labelsSelect(item)"
          >
            {{ item.EName }}
            <br v-if="item.EName" />
            {{ item.Name }}
            <div
              class="text-ellipsis overflow-hidden whitespace-nowrap"
              :title="item.NumberRange"
              v-if="item.NumberRange !== null"
            >
              {{ item.NumberRange }}
            </div>
          </div>
        </div>
        <div class="my-3" v-if="showTable">
          <q-table
            v-bind="QTableProps"
            :columns="columns"
            :rows="rows"
            row-key="Id"
            :pagination="pagination"
            :loading="tableLoading"
          >
            <template #body-cell-action="{ col, row }">
              <q-td :props="{ col }">
                <q-btn color="primary" label="帶入" dense unelevated @click="addProcedure(row)"></q-btn>
              </q-td>
            </template>
            <template v-slot:bottom>
              <my-pagination
                class="mx-auto p-4"
                v-model="pagination"
                v-on:update:model-value="GetProcedure(guideFlow.data[guideFlow.data.length - 1])"
              ></my-pagination>
            </template>
          </q-table>
        </div>
      </div>
    </my-dialog-content>
  </q-dialog>
</template>

<script setup>
import { ref, reactive, computed, watch, watchEffect } from 'vue'
import { useDialogPluginComponent, useQuasar } from 'quasar'
import { getRelationNhiSectDropdown, getRelationNhiItemDropdown, getProcedure, getRelationNhiSeqDropdown } from 'api'
import { QTableProps } from 'utils/quasar-extends/base-props.js'
import setSearchQuery from 'utils/setSearchQuery.js'
import { useAuthStore } from 'stores/auth'
import numberFormat from 'utils/numberFormat.js'

const { dialogRef, onDialogOK, onDialogCancel } = useDialogPluginComponent()
const authStore = useAuthStore()

const props = defineProps({
  pcsTableData: Array,
  underThisNhiChap: String,
})

const getDataLoading = ref(false)
const guideFlow = reactive({ data: [] })
const guideSelected = ref(0)

const quesFlow = reactive({ data: ['請點選檢查項目'] })

const labels = reactive({
  data: [],
})

const showTable = ref(false)
const tableLoading = ref(false)
const columns = [
  { name: 'NhiCode', label: '健保代碼', field: 'NhiCode', align: 'center' },
  { name: 'ProcedureCode', label: '院內代碼', field: 'ProcedureCode', align: 'center' },
  { name: 'ProcedureCname', label: '診療項目', field: (row) => row.ProcedureCname, align: 'center' },
  { name: 'AfterNhiPoint', label: '單價', field: (row) => numberFormat(row.AfterNhiPoint), align: 'center' },
  { name: 'action', label: '操作', field: 'action', align: 'center' },
]
const rows = ref([])
const pagination = ref({
  page: 1,
  rowsPerPage: 10,
  rowsNumber: 0,
})

const levelColor = (level) => {
  switch (level) {
    case 0: {
      return 'color-bg-purple'
    }
    case 1: {
      return 'color-bg-green'
    }
    case 2: {
      return 'color-bg-lightblue'
    }
    case 3: {
      return 'color-bg-pink'
    }
    case 4: {
      return 'color-bg-orange'
    }
    case 5: {
      return 'color-bg-brown'
    }
    case 6: {
      return 'color-bg-blue'
    }
  }
}

const GetRelationNhiSectDropdown = async () => {
  getDataLoading.value = true
  showTable.value = false
  labels.data = []
  try {
    const query = setSearchQuery({ code: props.underThisNhiChap })
    const res = await getRelationNhiSectDropdown(query)
    if (res.status === 200) {
      labels.data = res.data
    }
  } catch (err) {
    console.log(err)
  } finally {
    getDataLoading.value = false
  }
}
const GetRelationNhiItemDropdown = async (item) => {
  getDataLoading.value = true
  showTable.value = false
  labels.data = []
  try {
    const query = setSearchQuery({ code: item.No })
    const res = await getRelationNhiItemDropdown(query)
    if (res.status === 200) {
      if (res.data.length === 0) {
        GetProcedure(item)
      } else {
        quesFlow.data.push('選擇檢查分項')
        labels.data = res.data
      }
      labels.data = res.data
    }
  } catch (err) {
    console.log(err)
  } finally {
    getDataLoading.value = false
  }
}
const GetRelationNhiSeqDropdown = async (item) => {
  getDataLoading.value = true
  showTable.value = false
  labels.data = []
  try {
    const query = setSearchQuery({ code: item.No })
    const res = await getRelationNhiSeqDropdown(query)
    if (res.status === 200) {
      if (res.data.length === 0) {
        GetProcedure(item)
      } else {
        quesFlow.data.push('選擇檢查分序')
        labels.data = res.data
      }
    }
  } catch (err) {
    console.log(err)
  } finally {
    getDataLoading.value = false
  }
}

const guideFlowSelect = (index) => {
  guideSelected.value = index + 1
  if (index === 0) {
    GetRelationNhiItemDropdown(guideFlow.data[index])
  } else {
    GetRelationNhiSeqDropdown(guideFlow.data[index])
  }
}
const quesFlowSelect = (index, item) => {
  guideSelected.value = index
  if (index === 0) {
    GetRelationNhiSectDropdown()
  } else if (index === 1) {
    GetRelationNhiItemDropdown(guideFlow.data[index - 1])
  } else {
    GetRelationNhiSeqDropdown(guideFlow.data[index - 1])
  }
}

const labelsSelect = (item) => {
  pagination.value.page = 1
  guideFlow.data = guideFlow.data.slice(0, guideSelected.value)
  quesFlow.data = quesFlow.data.slice(0, guideSelected.value + 1)
  guideSelected.value++
  guideFlow.data = guideFlow.data.concat([item])
  if (guideSelected.value === 0) {
    GetRelationNhiSectDropdown(item)
  } else if (guideSelected.value === 1) {
    GetRelationNhiItemDropdown(item)
  } else if (guideSelected.value !== 3) {
    GetRelationNhiSeqDropdown(item)
  } else {
    GetProcedure(item)
  }
}

const GetProcedure = async (item) => {
  quesFlow.data.push('查詢結果')
  rows.value = []
  getDataLoading.value = true
  tableLoading.value = true
  showTable.value = true
  let filter = ''
  if (item.No.length === 9) {
    filter = `NhiSeq eq '${item.No}' and HospitalLevel eq '${authStore.organizeData.LevelId}'`
  } else if (item.No.length === 6) {
    filter = `NhiChap eq '${item.No.substring(0, 1)}' and NhiSect eq '${item.No.substring(0, 3)}' and NhiItem eq '${
      item.No
    }' and HospitalLevel eq '${authStore.organizeData.LevelId}'`
  } else if (item.No.length === 4) {
    filter = `NhiSect eq '${item.No}' and HospitalLevel eq '${authStore.organizeData.LevelId}'`
  } else if (item.No.length === 3) {
    filter = `NhiSect eq '${item.No}' and HospitalLevel eq '${authStore.organizeData.LevelId}'`
  }
  try {
    const query = setSearchQuery({
      $top: pagination.value.rowsPerPage,
      $skip: (pagination.value.page - 1) * pagination.value.rowsPerPage,
      $filter: filter,
    })
    const res = await getProcedure(query)
    if (res.status === 200) {
      rows.value = res.data.Items
      pagination.value.rowsNumber = res.data.Count
    }
  } catch (error) {
    console.log(error)
  } finally {
    getDataLoading.value = false
    tableLoading.value = false
  }
}

const addProcedure = (row) => {
  onDialogOK(row)
}

const init = () => {
  GetRelationNhiSectDropdown()
}
init()
</script>

<style lang="scss" scoped>
.hoverColor {
  &:hover {
    color: #0696ac;
  }
}

.labels-div {
  border: 1px solid #1d67dc;
  border-radius: 5px;
  color: white;
  cursor: pointer;
}

.color-bg-purple {
  background-color: #a05ef8;
  &:hover {
    background-color: #560fb4;
  }
}
.color-bg-green {
  background-color: #56b849;
  &:hover {
    background-color: #167e09;
  }
}
.color-bg-lightblue {
  background-color: #11bbd6;
  &:hover {
    background-color: #0696ac;
  }
}
.color-bg-pink {
  background-color: #e93592;
  &:hover {
    background-color: #950f54;
  }
}
.color-bg-orange {
  background-color: #f68132;
  &:hover {
    background-color: #c7601b;
  }
}
.color-bg-brown {
  background-color: #745145;
  &:hover {
    background-color: #442013;
  }
}
.color-bg-blue {
  background-color: #1868e1;
  &:hover {
    background-color: #063d91;
  }
}
</style>

