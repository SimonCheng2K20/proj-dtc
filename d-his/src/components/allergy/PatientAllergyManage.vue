<template>
  ...
  >
    <template #top>
      <div class="text-xl font-bold">藥品過敏記錄</div>
      <q-space></q-space>
    ...
    <template v-slot:bottom> <my-pagination class="mx-auto p-4" v-model="pagination" v-on:update:model-value="GetPatientAllergy"></my-pagination> </template
  ></q-table>
</template>
<script setup>
import { getPatientAllergy } from 'api'
import { ref, watch, computed } from 'vue'
import setSearchQuery from 'utils/setSearchQuery.js'
import { useQuasar } from 'quasar'
import EditItem from './PatientAllergyManageEditItem.vue'
...
const columns = [
  ...
  { name: 'MedicineId', label: '藥品代碼', field: 'MedicineId', align: 'left', headerClasses: 'min-w-[60px]' },
  { name: 'Name', label: '藥品名稱', field: (row) => row.EName || row.CName, align: 'left', headerClasses: 'min-w-[60px]' },
  ...
  { name: 'WriteDatetime', label: '建立日期', field: (row) => row.WriteDatetime?.substring(0, 10), align: 'left', headerClasses: 'min-w-[60px]' },
  { name: 'Remover', label: '移除者', field: 'Remover', align: 'left', headerClasses: 'min-w-[60px]' },
  { name: 'RemoveDatetime', label: '移除日期', field: (row) => row.RemoveDatetime?.substring(0, 10), align: 'left', headerClasses: 'min-w-[60px]' },
]

const tableLoading = ref(false)
const rows = ref([])
const GetPatientAllergy = async () => {
  try {
    ...
    const res = await getPatientAllergy(query)
    if (res.status === 200) {
