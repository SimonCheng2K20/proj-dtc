      ...
      <div class="pb-2">
        <my-input-group class="grid gap-2 grid-cols-4 p-2" style="width: 100%">
          ...
          <my-input-wrapper label="退庫單號">
            <q-input readonly bg-color="blue-grey-1" v-bind="QInputProps" v-model="editItem.BackoutId"></q-input>
          </my-input-wrapper>
          ...
          <my-input-wrapper v-if="editItem.Status === 0" label="庫房入庫人">
            <q-select
              readonly
              bg-color="blue-grey-1"
              v-bind="QSelectProps"
              ...
              emit-value
              map-options
              v-model="editItem.CheckAcceptPersonId"
            ></q-select>
          </my-input-wrapper>
        </my-input-group>
      </div>
...
// api
import { putStoreStockReturnCheckAccept, getDrugStore, getStoreStockReturnItem } from 'api'
...
const OnSaveClick = (remind) => {
  if (confirm(remind)) {
    PutStoreStockReturnCheckAccept()
  }
}
...
const PutStoreStockReturnCheckAccept = async () => {
  try {
    editLoading.value = true
    const body = {
      BackoutId: editItem.BackoutId,
      CheckAcceptPersonId: editItem.CheckAcceptPersonId,
    }
    const res = await putStoreStockReturnCheckAccept(body)
    ...
    $q.notify({
      type: 'negative',
      message: '退庫單入庫失敗：' + typeof err.data === 'string' ? err.data : JSON.stringify(err.data),
      position: 'top',
    })
  } finally {
...
const init = async () => {
  await GetDrugStore()
  await GetStoreStockReturnItem()
  if (props.editType === 'edit') {
    editItem.CheckAcceptPersonId = authStore.userData.No
  }
}
