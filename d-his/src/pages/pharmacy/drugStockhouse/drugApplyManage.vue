<template>
  <q-page>
    <my-title no-fixed title="藥房申領單維護"></my-title>
    <div class="p-2">
      <my-search-area grid-cols="3" @search-submit="searchSubmit" @search-clear="searchClear">
        ...
        <my-input-wrapper label="庫房出庫人">
          <q-select
            v-bind="QSelectProps"
            v-model="search.ShipPersonId"
            :options="options"
            @update:model-value="searchSubmit"
          >
          </q-select>
...
// components
import Edit from './components/drugApplyManageEdit.vue'
...
const columns = reactive([
  ...
  { name: 'StatusName', label: '訂單狀態', field: 'StatusName', align: 'left' },
  {
    name: 'ShipDate',
    label: '申領單出庫完成時間',
    field: (row) => (dayjs(row.ShipDate).isValid() ? dayjs(row.ShipDate).format('YYYY-MM-DD') : row.ShipDate),
    align: 'left',
  },
  { name: 'PersonName', label: '申領人員', field: 'PersonName', align: 'left' },
  { name: 'ShipPersonName', label: '庫房出庫人', field: 'ShipPersonName', align: 'left' },
])
...
const filter = computed(() => {
  ...
  if (!str.includes('Status eq')) {
    if (str) str += ' and Status lt 2'
    else str += 'Status lt 2'
  }
  return str
})
