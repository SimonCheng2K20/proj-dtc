<template>
  <q-dialog ref="dialogRef" no-backdrop-dismiss>
    <my-dialog-content :header="editHeader" width="1280px" @close="onDialogCancel" :loading="getDataLoading">
      <div v-if="editType === 'delete'" class="text-negative mb-1">是否確定刪除此註記？</div>
      <my-input-group class="grid gap-2">
        <my-input-wrapper label="註記代碼" required-mark>
          <q-input
            v-bind="QInputProps"
      ...
      <template #actions>
        ...
        <q-btn
          ...
          :loading="editLoading"
          :disable="getDataLoading"
          @click="DeleteCommonDataItem(`Category(InpatientNote),No(${editItem.No})`)"
        ></q-btn>
      </template>
    </my-dialog-content>
  </q-dialog>
...
const PostCommonDataItem = async (editItem) => {
  try {
    ...
    $q.notify({
      type: 'negative',
      message: '新增註記失敗：' + typeof error.data === 'string' ? error.data : JSON.stringify(error.data),
      position: 'top',
    })
  } finally {
...
const PutCommonDataItem = async (editItem) => {
  try {
    ...
    $q.notify({
      type: 'negative',
      message: '編輯註記失敗：' + typeof error.data === 'string' ? error.data : JSON.stringify(error.data),
      position: 'top',
    })
  } finally {
    editLoading.value = false
...
const DeleteCommonDataItem = async (id) => {
  try {
    ...
    $q.notify({
      type: 'negative',
      message: '刪除註記失敗：' + typeof error.data === 'string' ? error.data : JSON.stringify(error.data),
      position: 'top',
    })
  } finally {
    editLoading.value = false
