...
import EditItem from './components/StayHospitalRemarkManageEditItem.vue'
...
const columns = [
  ...
  { name: 'No', label: '註記代碼', field: 'No', align: 'left' },
  { name: 'Text', label: '註記名稱', field: 'Text', align: 'left' },
]
...
const filter = computed(() => {
  ...
  return str ? str + ` and Category eq 'InpatientNote'` : `Category eq 'InpatientNote'`
})
...
const toEdit = (row) => {
  const dialog = $q
    .dialog({
      ...
      componentProps: {
        editType: 'edit',
        id: `Category(InpatientNote),No(${row.No})`,
      },
    })
    .onOk(() => {
      dialog.hide()
