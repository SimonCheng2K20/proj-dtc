<template>
  <div>
    <my-title noFixed title="收費項目"></my-title>
    <div class="flex flex-wrap py-2">
      <DtxInputGroup class="mr-2 mb-2" title="收費項目代碼">
        <InputText v-model="search.code" class="w-full" />
      </DtxInputGroup>
      <DtxInputGroup class="mr-2 mb-2" title="收費項目名稱">
        <InputText v-model="search.name" class="w-full" />
      </DtxInputGroup>

      <div class="flex mb-2">
        <Button class="mr-2" label="進行查詢" icon="pi pi-search" @click="searchSubmit" />
        <Button class="p-button-secondary mr-2" label="清除查詢" icon="pi pi-undo" @click="searchClear" />
        <Button label="新增資料" class="btn-add" icon="pi pi-plus" @click="toAdd" />
      </div>
    </div>

    <v-data-table :headers="headers" :items="items" :pageOptions="pageOptions" @pageChange="pageChange">
      <template #item-action="{ item }">
        <div class="flex justify-center gap-3">
          <Button class="p-button-sm" @click="toEdit(item)" label="編輯" icon="pi pi-pencil" />
          <Button class="p-button-warning p-button-sm" label="刪除" @click="toDelete(item)" icon="pi pi-trash" />
        </div>
      </template>
    </v-data-table>

    <Dialog v-model:visible="dialog.edit.show" :draggable="false" modal :header="editHeader" style="width: 768px">
      <div v-if="dialog.edit.type === 'delete'" class="font-bold">是否確定刪除此收費項目？</div>
      <DtxInputGroup title="收費項目代碼">
        <InputText v-model="editItem.code" class="w-full" :disabled="dialog.edit.type !== 'add'" />
      </DtxInputGroup>
      <DtxInputGroup title="收費項目名稱" class="mt-2">
        <InputText v-model="editItem.name" class="w-full" :disabled="dialog.edit.type === 'delete'" />
      </DtxInputGroup>
      <DtxInputGroup title="收據類別" class="mt-2">
        <Dropdown
          v-model="editItem.receiptCode"
          :options="receiptDropdown"
          optionLabel="name"
          optionValue="code"
          class="w-full"
          :disabled="dialog.edit.type === 'delete'"
        />
      </DtxInputGroup>

      <template #footer>
        <Button class="p-button-grey" @click="dialog.edit.show = false">取消</Button>
        <Button v-if="dialog.edit.type === 'add'" @click="PostCharge">建立</Button>
        <Button v-if="dialog.edit.type === 'edit'" @click="PutCharge">儲存</Button>
        <Button v-if="dialog.edit.type === 'delete'" class="p-button-warning" @click="DeleteCharge">刪除</Button>
      </template>
    </Dialog>
  </div>
</template>

<script setup>
import { ref, reactive, computed } from 'vue'
import _ from 'lodash'
import { useQuasar } from 'quasar'

const $q = useQuasar()
const headers = reactive([
  { text: '操作', value: 'action', width: '170px' },
  { text: '收費項目代碼', value: 'code' },
  { text: '收費項目名稱', value: 'name' },
  { text: '收據類別名稱', value: 'receiptName' },
])

const items = ref([])
const GetCharge = async (query = '') => {
  try {
    const res = await getCharge(query)

    switch (res.status) {
      case 200:
        items.value = res.data.entry
        pageOptions.totalItemsCount = res.data.count
        break
      default: {
        const errText = await res.text()
        throw Error(`${res.status}: ${errText}`)
      }
    }
  } catch (e) {
    console.log(e)
  }
}

const pageOptions = reactive({
  firstIndex: 0,
  perPageCount: 10,
  totalItemsCount: 0,
  rowsPerPage: [10, 20, 30],
})

const search = reactive({
  code: '',
  name: '',
  receiptCode: '',
})

const pageChange = (e) => {
  const query = `?pageStart=${e.first}&limit=${pageOptions.perPageCount}&${searchQuery.value}`
  GetCharge(query)
}

const searchQuery = computed(() => {
  const queryArr = []
  _.forEach(Object.entries(search), ([key, value]) => {
    queryArr.push(`${key}=${value}`)
  })
  return _.join(queryArr, '&')
})

const searchSubmit = () => {
  const query = `?limit=${pageOptions.perPageCount}&${searchQuery.value}`
  GetCharge(query)
  pageOptions.firstIndex = 0
}

const searchClear = () => {
  const query = `?limit=${pageOptions.perPageCount}`
  _.forEach(Object.keys(search), (key) => {
    search[key] = ''
  })
  GetCharge(query)
  pageOptions.firstIndex = 0
}

const dialog = reactive({
  edit: {
    show: false,
  },
})

const editItem = reactive({
  code: '',
  name: '',
})

const toAdd = () => {
  _.forEach(Object.keys(editItem), (key) => {
    editItem[key] = ''
  })

  dialog.edit.type = 'add'
  dialog.edit.show = true
}

const toEdit = (item) => {
  _.forEach(Object.keys(editItem), (key) => {
    editItem[key] = item[key]
  })

  dialog.edit.type = 'edit'
  dialog.edit.show = true
}

const toDelete = (item) => {
  _.forEach(Object.keys(editItem), (key) => {
    editItem[key] = item[key]
  })

  dialog.edit.type = 'delete'
  dialog.edit.show = true
}

const editHeader = computed(() => {
  switch (dialog.edit.type) {
    case 'add':
      return '新增收費項目'
    case 'edit':
      return '編輯收費項目'
    case 'delete':
      return '刪除收費項目'
    default:
      return ''
  }
})

const PostCharge = async () => {
  try {
    const res = await postCharge(editItem)

    switch (res.status) {
      case 200:
        $q.notify({
          type: 'positive',

          message: '新增收費項目成功',
          position: 'top',
        })
        dialog.edit.show = false
        GetCharge(`?limit=${pageOptions.perPageCount}`)
        pageOptions.firstIndex = 0
        break
      default: {
        const errText = await res.text()
        throw Error(`${res.status}: ${errText}`)
      }
    }
  } catch (e) {
    console.log(e)
    $q.notify({
      type: 'negative',

      message: '新增收費項目失敗',
      position: 'top',
    })
  }
}

const PutCharge = async () => {
  try {
    const res = await putCharge(editItem)

    switch (res.status) {
      case 200:
        $q.notify({
          type: 'positive',

          message: '編輯收費項目成功',
          position: 'top',
        })
        dialog.edit.show = false
        GetCharge(`?pageStart=${pageOptions.firstIndex}&limit=${pageOptions.perPageCount}&${searchQuery.value}`)
        break
      default: {
        const errText = await res.text()
        throw Error(`${res.status}: ${errText}`)
      }
    }
  } catch (err) {
    console.log(err)
    $q.notify({
      type: 'negative',

      message: '編輯收費項目失敗',
      position: 'top',
    })
  }
}

const DeleteCharge = async () => {
  try {
    const res = await deleteCharge(editItem)

    switch (res.status) {
      case 200:
        $q.notify({
          type: 'positive',

          message: '刪除收費項目成功',
          position: 'top',
        })
        dialog.edit.show = false
        GetCharge(`?pageStart=${pageOptions.firstIndex}&limit=${pageOptions.perPageCount}&${searchQuery.value}`)
        break
      default: {
        const errText = await res.text()
        throw Error(`${res.status}: ${errText}`)
      }
    }
  } catch (err) {
    console.log(err)
    $q.notify({
      type: 'negative',

      message: '刪除收費項目失敗',
      position: 'top',
    })
  }
}

const receiptDropdown = ref([])
const GetReceipt = async () => {
  try {
    const res = await getReceipt()

    switch (res.status) {
      case 200:
        receiptDropdown.value = res.data.entry
        break
      default: {
        const errText = await res.text()
        throw Error(`${res.status}: ${errText}`)
      }
    }
  } catch (e) {
    console.log(e)
  }
}

// init
;(() => {
  const query = `?limit=${pageOptions.perPageCount}`
  GetCharge(query)
  GetReceipt()
})()
</script>
