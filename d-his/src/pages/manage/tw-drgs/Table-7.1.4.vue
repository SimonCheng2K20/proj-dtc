<template>
  <q-page>
    <my-title title="合併症或併發症診斷碼（附表7.1.4）" no-fixed></my-title>
    <div class="p-2">
      <my-search-area grid-cols="3" v-on:search-submit="searchSubmit" v-on:search-clear="searchClear">
        ...
        <my-input-wrapper label="次診斷英文名稱">
      ...
      <q-table class="mt-2" v-bind="QTableProps" :rows="rows" :columns="columns" row-key="twdrgid" :pagination="pagination" :loading="tableLoading">
        <template v-slot:bottom> <my-pagination class="mx-auto p-4" v-model="pagination" v-on:update:model-value="GetIcd10Pcs714"></my-pagination> </template>
      </q-table>
    </div>
  </q-page>
</template>
<script setup>
import { getIcd10Pcs714 } from 'api'
...
const columns = [
  ...
  { name: 'SecName', label: '次診斷英文名稱', field: 'SecName', align: 'left' },
]

const tableLoading = ref(false)
const rows = ref([])
const GetIcd10Pcs714 = async () => {
  try {
    ...
    const res = await getIcd10Pcs714(query)
    if (res.status === 200) {
