<template>
  <q-page>
    <my-title title="合併症或併發症主診斷碼（附表7.1.4.2）" no-fixed></my-title>
    <div class="p-2">
      <my-search-area grid-cols="2" v-on:search-submit="searchSubmit" v-on:search-clear="searchClear">
        <my-input-wrapper label="主診斷代碼"><q-input v-bind="QInputProps" v-model="search.Main" @keydown.enter="searchSubmit"></q-input></my-input-wrapper>
      ...
      <q-table class="mt-2" v-bind="QTableProps" :rows="rows" :columns="columns" row-key="twdrgid" :pagination="pagination" :loading="tableLoading">
        <template v-slot:bottom> <my-pagination class="mx-auto p-4" v-model="pagination" v-on:update:model-value="GetIcd10Pcs7142"></my-pagination> </template>
      </q-table>
    </div>
  </q-page>
</template>
<script setup>
import { getIcd10Pcs7142 } from 'api'
...
const GetIcd10Pcs7142 = async () => {
  try {
    ...
    const res = await getIcd10Pcs7142(query)
    if (res.status === 200) {
