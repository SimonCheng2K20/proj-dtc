<template>
  <q-page>
    <my-title title="MDC8 開放性、閉鎖性骨折診斷碼（附表7.1.12）" no-fixed></my-title>
    <div class="p-2">
      <my-search-area grid-cols="4" v-on:search-submit="searchSubmit" v-on:search-clear="searchClear">
        <my-input-wrapper label="代號"><q-input v-bind="QInputProps" v-model="search.Remark" @keydown.enter="searchSubmit"></q-input></my-input-wrapper>
        <my-input-wrapper label="性質"><q-input v-bind="QInputProps" v-model="search.性質" @keydown.enter="searchSubmit"></q-input></my-input-wrapper>
      ...
      <q-table class="mt-2" v-bind="QTableProps" :rows="rows" :columns="columns" row-key="twdrgid" :pagination="pagination" :loading="tableLoading">
        <template v-slot:bottom> <my-pagination class="mx-auto p-4" v-model="pagination" v-on:update:model-value="GetIcd10Pcs7112"></my-pagination> </template>
      </q-table>
    </div>
  </q-page>
</template>
<script setup>
import { getIcd10Pcs7112 } from 'api'
...
const columns = [
  { name: 'Remark', label: '代號', field: 'Remark', align: 'left' },
  { name: '性質', label: '性質', field: '性質', align: 'left' },
...
const GetIcd10Pcs7112 = async () => {
  try {
    ...
    const res = await getIcd10Pcs7112(query)
    if (res.status === 200) {
