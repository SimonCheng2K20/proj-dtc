<template>
  <q-page>
    <my-title title="檢體維護" no-fixed></my-title>
    <div class="p-2">
      <my-search-area grid-cols="2" v-on:search-submit="searchSubmit" v-on:search-clear="searchClear">
        <my-input-wrapper label="檢體代碼"><q-input v-bind="QInputProps" v-model="search.No" @keydown.enter="searchSubmit"></q-input></my-input-wrapper>
        <my-input-wrapper label="檢體名稱"> <q-input v-bind="QInputProps" v-model="search.Text" @keydown.enter="searchSubmit"></q-input></my-input-wrapper>
...
import EditItem from './components/SpecimenEditItem.vue'
...
const columns = [
  ...
  { name: 'No', label: '檢體代碼', field: 'No', align: 'left' },
  { name: 'Text', label: '檢體名稱', field: 'Text', align: 'left' },
]
...
const filter = computed(() => {
  ...
  return str ? str + ` and Category eq 'specimen'` : `Category eq 'specimen'`
})
...
const toEdit = (row) => {
  const dialog = $q
    .dialog({
      ...
      componentProps: {
        editType: 'edit',
        id: `Category(specimen),No(${row.No})`,
      },
    })
    .onOk(() => {
      dialog.hide()
