...
const PostCommonDataItem = async (editItem) => {
  try {
    ...
    $q.notify({
      type: 'negative',
      message: '新增檢體失敗：' + typeof err.data === 'string' ? err.data : JSON.stringify(err.data),
      position: 'top',
    })
  } finally {
...
const PutCommonDataItem = async (editItem) => {
  try {
    ...
    $q.notify({
      type: 'negative',
      message: '編輯檢體失敗：' + typeof err.data === 'string' ? err.data : JSON.stringify(err.data),
      position: 'top',
    })
  } finally {
    editLoading.value = false
...
const DeleteCommonDataItem = async () => {
  try {
    ...
    $q.notify({
      type: 'negative',
      message: '刪除檢體失敗：' + typeof err.data === 'string' ? err.data : JSON.stringify(err.data),
      position: 'top',
    })
  } finally {
    editLoading.value = false
