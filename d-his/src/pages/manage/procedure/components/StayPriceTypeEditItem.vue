...
const rules = {
  No: [(val) => (val !== null && null !== '') || '請輸入住院費用類別代碼'],
}
...
const PostCommonDataItem = async (editItem) => {
  try {
    ...
    $q.notify({
      type: 'negative',
      message: '新增住院費用類別失敗：' + typeof err.data === 'string' ? err.data : JSON.stringify(err.data),
      position: 'top',
    })
  } finally {
...
const PutCommonDataItem = async (editItem) => {
  try {
    ...
    $q.notify({
      type: 'negative',
      message: '編輯住院費用類別失敗：' + typeof err.data === 'string' ? err.data : JSON.stringify(err.data),
      position: 'top',
    })
  } finally {
    editLoading.value = false
...
const DeleteCommonDataItem = async () => {
  try {
    ...
    $q.notify({
      type: 'negative',
      message: '刪除住院費用類別失敗：' + typeof err.data === 'string' ? err.data : JSON.stringify(err.data),
      position: 'top',
    })
  } finally {
    editLoading.value = false
