...
const rules = {
  No: [(val) => (val !== null && null !== '') || '請輸入診療部位代碼'],
}
...
const PostCommonDataItem = async (editItem) => {
  try {
    ...
    $q.notify({
      type: 'negative',
      message: '新增診療部位失敗：' + typeof err.data === 'string' ? err.data : JSON.stringify(err.data),
      position: 'top',
    })
  } finally {
...
const PutCommonDataItem = async (editItem) => {
  try {
    ...
    $q.notify({
      type: 'negative',
      message: '編輯診療部位失敗：' + typeof err.data === 'string' ? err.data : JSON.stringify(err.data),
      position: 'top',
    })
  } finally {
    editLoading.value = false
...
const DeleteCommonDataItem = async () => {
  try {
    ...
    $q.notify({
      type: 'negative',
      message: '刪除診療部位失敗：' + typeof err.data === 'string' ? err.data : JSON.stringify(err.data),
      position: 'top',
    })
  } finally {
    editLoading.value = false
