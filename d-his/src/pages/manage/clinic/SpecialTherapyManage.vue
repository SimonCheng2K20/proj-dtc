<template>
  <q-page>
    <my-title title="特定治療管理" no-fixed></my-title>
    <div class="p-2">
      <my-search-area grid-cols="3" v-on:search-submit="searchSubmit" v-on:search-clear="searchClear">
        <my-input-wrapper label="特定治療項代號欄位(一)">
          <q-input v-bind="QInputProps" v-model="search.CureItemNo1" @keydown.enter="searchSubmit"> </q-input>
        </my-input-wrapper>
        <my-input-wrapper label="特定治療項代號欄位(二)">
          <q-input v-bind="QInputProps" v-model="search.CureItemNo2" @keydown.enter="searchSubmit"> </q-input>
        </my-input-wrapper>
        <my-input-wrapper label="特定治療項代號欄位(三)">
          <q-input v-bind="QInputProps" v-model="search.CureItemNo3" @keydown.enter="searchSubmit"> </q-input>
        </my-input-wrapper>
        <template #btn-expand>
      ...
      >
        ...
        <template v-slot:bottom>
          <my-pagination class="mx-auto p-4" v-model="pagination" v-on:update:model-value="GetSpecialTherapy">
          </my-pagination>
        </template>
      </q-table>
    </div>
...
import EditItem from './components/SpecialTherapyEditItem.vue'
...
const columns = [
  ...
  { name: 'Sn', label: '序號', field: 'Sn', align: 'center' },
  { name: 'PlanText', label: '計畫情形', field: 'PlanText', align: 'center' },
  { name: 'CureItemNo1', label: '特定治療項代號欄位(一)', field: 'CureItemNo1', align: 'center' },
  { name: 'CureItemNo2', label: '特定治療項代號欄位(二)', field: 'CureItemNo2', align: 'center' },
  { name: 'CureItemNo3', label: '特定治療項代號欄位(三)', field: 'CureItemNo3', align: 'center' },
