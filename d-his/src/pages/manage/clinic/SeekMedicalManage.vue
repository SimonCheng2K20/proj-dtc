...
import SeekMedicalEdit from './components/SeekMedicalEdit.vue'
...
const columns = [
  ...
  { name: 'MedType', label: '就醫序號', field: (row) => (row.MedType === 0 ? '不累計' : '累計'), align: 'left' },
]
...
const filter = computed(() => {
  ...
  return str ? str + ` and Category eq 'SeekMedical'` : `Category eq 'SeekMedical'`
})
...
const toAdd = () => {
  $q.dialog({
    component: SeekMedicalEdit,
    componentProps: {
      editType: 'add',
      category: 'SeekMedical',
    },
  }).onOk(() => {
    pagination.page = 1
    GetCommonData()
