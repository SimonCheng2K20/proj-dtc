<template>
  <q-page>
    <my-title title="部分負擔管理" no-fixed></my-title>
    <div class="p-2">
      ...
      >
        ...
        <template v-slot:bottom>
          <my-pagination class="mx-auto p-4" v-model="pagination" v-on:update:model-value="GetPartialAfford">
          </my-pagination>
        </template>
      </q-table>
    </div>
...
<script setup>
import { getPartialAfford } from 'api'
...
import EditItem from './components/PartialAffordEditItem.vue'
...
const columns = [
  ...
  { name: 'Code', label: '部分負擔代碼', field: 'Code', align: 'left' },
  { name: 'Name', label: '部分負擔名稱', field: 'Name', align: 'left' },
  { name: 'Amount', label: '應收部分負擔金額', field: (row) => numberFormat(row.Amount), align: 'right' },
  { name: 'Actived', label: '啟用狀態', field: 'Actived', align: 'left' },
]

const tableLoading = ref(false)
const rows = ref([])
const GetPartialAfford = async () => {
  try {
    ...
    const res = await getPartialAfford(query)
