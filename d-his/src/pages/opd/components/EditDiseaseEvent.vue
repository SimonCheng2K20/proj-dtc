<template>
  <q-dialog ref="dialogRef" no-backdrop-dismiss full-width>
    <my-dialog-content :header="editHeader" width="540px" @close="onDialogCancel" :loading="getDataLoading">
      <div>
...
const init = () => {
  if (!!props.Id || props.hasTrace === false) {
...
const onSaveOrDeleteClick = () => {
  const cloneEditItem = cloneDeep(editItem)

  if (props.hasTrace === false) {
    cloneEditItem.Step = props.step
    onDialogOK(cloneEditItem)
  } else {
