<template>
  <q-page class="p-2">
    <my-title title="居家照護申請案件"></my-title>
    <my-search-area grid-cols="4" v-on:search-submit="searchSubmit" v-on:search-clear="searchClear">
      ...
      <my-input-wrapper label="申請書狀態">
        <q-select
          v-bind="QSelectProps"
          v-model="search.申請書狀態"
          :options="undefined"
      ...
      <template #btn-expand>
        <q-btn unelevated color="secondary" icon="add" label="建立申請書" @click="toAdd"> </q-btn>
      </template>
    </my-search-area>
    ...
    >
      ...
      <template v-slot:bottom>
        <my-pagination class="mx-auto p-4" v-model="pagination" v-on:update:model-value="GetHomeCareApply">
        </my-pagination>
      </template>
    </q-table>
  </q-page>
...
<script setup>
// import { getHomeCareApply } from 'api'
...
import EditItem from './components/HomeCareApplyEditItem.vue'
...
const columns = [
  ...
  { name: '申請類別', label: '申請類別', field: '申請類別', align: 'left' },
  ...
  { name: '前次核定迄日', label: '前次核定迄日', field: '前次核定迄日', align: 'left' },
  ...
  { name: '申請書狀態', label: '申請書狀態', field: '申請書狀態', align: 'left' },
  { name: '異動日期', label: '異動日期', field: '異動日期', align: 'left' },
]

const tableLoading = ref(false)
const rows = ref([])
const GetHomeCareApply = async () => {
  try {
    ...
    const res = await getHomeCareApply(query)
