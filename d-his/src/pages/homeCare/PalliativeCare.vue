<template>
  <q-page class="p-2">
    ...
    <my-search-area grid-cols="3" v-on:search-submit="searchSubmit" v-on:search-clear="searchClear">
      ...
      <my-input-wrapper label="申請單狀態">
        <q-select v-bind="QSelectProps" v-model="search.申請單狀態" @keydown.enter="searchSubmit"> </q-select>
      </my-input-wrapper>
    ...
    >
      ...
      <template v-slot:bottom>
        <my-pagination class="mx-auto p-4" v-model="pagination" v-on:update:model-value="GetPalliativeCare">
        </my-pagination>
      </template>
    </q-table>
  </q-page>
...
// api
// import { getPalliativeCare } from 'api'

// components
import EditItem from './components/PalliativeCareEditItem.vue'
...
const GetPalliativeCare = async () => {
  try {
    ...
    const res = await getPalliativeCare(query)
