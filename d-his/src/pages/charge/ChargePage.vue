    ...
    <my-search-area grid-cols="4" @search-submit="searchSubmit" @search-clear="searchClear">
      ...
      <transition name="more-search">
        <div class="col-span-full grid grid-cols-3 gap-2" v-show="isMoreFilter">
          ...
          <my-input-wrapper label="看診日期區間" key="看診日期" range-input>
...
const GetCharge = async () => {
  ...
  let filter = null
  if (chargeStatus.value !== '6') {
    filter = "OpdCharge/ChargeStatus eq '8' or OpdCharge/ChargeStatus eq '9'"
  } else {
    filter = "OpdCharge/ChargeStatus eq '6'"
  }
  try {
...
const onReadCard = async (data) => {
  if (data) {
    ...
    router.push({ name: 'ChargingThePatient', query: { PersonId: data.PersonId } })
  }
}
...
const toChargingThePatient = (row) => {
  router.push({ name: 'ChargingThePatient', query: { PersonId: row.RoomBooking.Patient.PersonId } })
}
