<template>
  <div class="BasicInfo">
    <Dialog v-model="showDialog" :title="isAdd ? '新增敏感身分類別':'編輯敏感身分類別'" :width="'40vw'">
      <el-form ref="postform" label-position="left" :model="formCopy" :rules="rules" label-width="140px" @keyup.enter.native="sub">
        <InputLabel class="mb-1" title="敏感身分代碼" prop="PatientClassId">
          <el-input v-model="formCopy.PatientClassId" />
        </InputLabel>
        <InputLabel class="mb-1" title="敏感身分名稱" prop="PatientClassName">
          <el-input v-model="formCopy.PatientClassName" />
        </InputLabel>
    ...
    <CardBox class="mb-1" title="敏感身分類別維護">
      <el-form label-position="left" :model="tableDataFormCopy" label-width="130px" @keyup.enter.native="getData">
        <el-row :gutter="10">
          <el-col :md="6">
            <!-- <InputLabel class="mb-1" title="啟用狀態">
            ...
            <InputLabel class="mb-1" title="啟用狀態">
              <el-radio-group class="ml-1" v-model="tableDataFormCopy.OnNotGrant">
                <el-radio label="0">啟用</el-radio>
                <el-radio label="1">停用</el-radio>
              </el-radio-group>
            </InputLabel>
          ...
          <el-col :md="6">
            <InputLabel class="mb-1" title="敏感身分代碼">
              <el-input v-model="tableDataFormCopy.PatientClassId" />
            </InputLabel>
          </el-col>
          <el-col :md="6">
            <InputLabel class="mb-1" title="敏感身分名稱">
              <el-input v-model="tableDataFormCopy.PatientClassName" />
            </InputLabel>
          </el-col>
          <el-col :md="6">
      ...
      <div class="d-flex justify-content-end">
        ...
        <Button class="buttonColor-6" @click="add">新增敏感身分類別</Button>
      </div>
    </CardBox>
    <CardBox class="mb-1" v-if="tableData">
      <el-table ref="table" class="mb-1" :default-sort="{prop:tableDataFormCopy.sortKey,order:tableDataFormCopy.sortOrder}" :data="tableData.Items" border stripe style="width: 100%" @sort-change="sortChange">
        ...
        <el-table-column prop="PatientClassId" label="敏感身分代碼" sortable="custom"></el-table-column>
        <el-table-column prop="PatientClassName" label="敏感身分名稱" sortable="custom"></el-table-column>
        <el-table-column prop="WrapWording" label="顯示用字" sortable="custom"></el-table-column>
        <el-table-column prop="IsPerm" label="永久保存" sortable="custom">
          <template slot-scope="scope">
            <el-checkbox v-model="scope.row.IsPerm" true-label="Y" false-label="N" @change="changeActive(scope.row)" />
          </template>
        </el-table-column>
        <el-table-column prop="Interchange" label="允許交換" sortable="custom">
          <template slot-scope="scope">
            <el-checkbox v-model="scope.row.Interchange" true-label="Y" false-label="N" @change="changeActive(scope.row)" />
          </template>
        </el-table-column>
        ...
        <el-table-column prop="OnNotGrant" label="啟用狀態" sortable="custom">
          <template slot-scope="scope">
            <el-checkbox v-model="scope.row.OnNotGrant" true-label="0" false-label="1" @change="changeActive(scope.row)" />
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
export default {
  ...
  methods: {
    async getData() {
      let filter=[]
      if(this.tableDataFormCopy.OnNotGrant) filter.push(`OnNotGrant eq '${this.tableDataFormCopy.OnNotGrant}'`)
      if(this.tableDataFormCopy.PatientClassId) filter.push(`contains(PatientClassId,'${this.tableDataFormCopy.PatientClassId}')`)
      if(this.tableDataFormCopy.PatientClassName) filter.push(`contains(PatientClassName,'${this.tableDataFormCopy.PatientClassName}')`)
      if(this.tableDataFormCopy.WrapWording) filter.push(`contains(WrapWording,'${this.tableDataFormCopy.WrapWording}')`)
      ...
      this.tableData=await this.$api.getPatientClass(data)
      this.$store.commit('patientClass',await this.$api.getPatientClass().then(res=>res.Items))
    ...
    async changeActive(x) {
      ...
      await this.$api.putPatientClass(x)
      await this.getData()
    ...
    async del(x) {
      ...
      this.$store.commit('loading',true)
      await this.$api.delPatientClass(x.PatientClassOid)
      await this.getData()
      this.$store.commit('loading',false)
    ...
    async sub() {
      this.$refs.postform.validate(async (valid) => {
        if(valid) {
          this.$store.commit('loading',true)
          if(this.isAdd){
            await this.$api.postPatientClass(this.formCopy)
          }else {
            await this.$api.putPatientClass(this.formCopy)
          }
          await this.getData()
