  ...
  <div class="BasicInfo">
    ...
    >
      ...
      >
        <InputLabel class="mb-1" title="病歷號/病患姓名" prop="SheetIndexOid">
          ...
          >
            ...
            <el-option
              ...
              :label="`病歷號：${item.ChartNo} / 姓名：${item.PatientName} / 身分證字號：${item.IdNo}`"
              :value="item.SheetIndexOid"
            >
            </el-option>
          </el-select>
        </InputLabel>
        <InputLabel class="mb-1" title="敏感分類" prop="SheetEventClassOid">
          <el-select
            v-model="formCopy.SheetEventClassOid"
            :disabled="!isAdd"
            style="width: 100%"
          >
            <el-option
              v-for="item in patientEventClassSelects"
              :key="item.SheetEventClassOid"
              :label="item.SheetEventClassName"
              :value="item.SheetEventClassOid"
            ></el-option>
          </el-select>
        </InputLabel>
        <InputLabel class="mb-1" title="就診科別" prop="SheetEventDeptId">
          <el-select
            v-model="formCopy.SheetEventDeptId"
            :disabled="!isAdd"
            style="width: 100%"
          >
            <el-option
        ...
        <el-row :gutter="10">
          <el-col :md="12">
            <InputLabel class="mb-1" title="就診時間起">
              <el-date-picker
                type="date"
                value-format="yyyyMMdd"
                v-model="formCopy.SheetEventFromDate"
                :disabled="!isAdd"
              />
            </InputLabel>
          </el-col>
          <el-col :md="12">
            <InputLabel class="mb-1" title="就診時間迄">
              <el-date-picker
                type="date"
                value-format="yyyyMMdd"
                v-model="formCopy.SheetEventEndDate"
                :disabled="!isAdd"
              />
            </InputLabel>
          </el-col>
    ...
    <CardBox class="mb-1" title="敏感病歷維護">
      <el-form
        label-position="left"
      ...
      >
        <el-row :gutter="10">
          ...
          <el-col :md="10">
            <InputLabel class="mb-1" title="就診時間">
              <el-date-picker
                v-model="tableDataFormCopy.SheetEventDate"
                type="daterange"
                range-separator="~"
                value-format="yyyyMMdd"
              />
          ...
          <el-col :md="10">
            <InputLabel class="mb-1" title="就診科別">
              <el-select
                v-model="tableDataFormCopy.SheetEventDeptId"
                style="width: 100%"
              >
                <el-option
                  v-for="item in $store.state.deptList"
          ...
          <el-col :md="10">
            <InputLabel class="mb-1" title="敏感分類">
              <el-select
                v-model="tableDataFormCopy.SheetEventClassOid"
                style="width: 100%"
              >
                <el-option
                  v-for="item in patientEventClassSelects"
    ...
    <CardBox class="mb-1" v-if="tableData">
      ...
      >
        ...
        <el-table-column prop="SheetEventDeptId" label="就診科別">
          <template slot-scope="scope">
            {{
              $store.getters.deptListObj[scope.row.SheetEventDeptId] &&
        ...
        <el-table-column prop="SheetEventClassOid" label="敏感分類">
          <template slot-scope="scope">
            {{
              $store.getters.sheetEventClassObj[scope.row.SheetEventClassOid] &&
              $store.getters.sheetEventClassObj[scope.row.SheetEventClassOid]
                .SheetEventClassName
            }}
          </template>
        </el-table-column>
        <el-table-column prop="SheetEventFromDate" label="就診時間起">
          <template slot-scope="scope">
            {{
              dayjs(scope.row.SheetEventFromDate).isValid()
                ? dayjs(scope.row.SheetEventFromDate).format("YYYY-MM-DD")
                : null
            }}
          </template>
        </el-table-column>
        <el-table-column prop="SheetEventEndDate" label="就診時間迄">
          <template slot-scope="scope">
            {{
              dayjs(scope.row.SheetEventEndDate).isValid()
                ? dayjs(scope.row.SheetEventEndDate).format("YYYY-MM-DD")
                : null
            }}
          </template>
        </el-table-column>
...
export default {
  ...
  async created() {
    ...
    await this.getSheetEventClassSelect();
    await this.getData();
  },
  methods: {
    async getSheetEventClassSelect() {
      let filter = [];
      ...
      let res = await this.$api.getSheetEventClass(data);
      if (res) {
        this.patientEventClassSelects = res.Items;
      }
    },
    async getData() {
      let filter = [];
      ...
      if (this.tableDataFormCopy.SheetEventDeptId)
        filter.push(
          `SheetEventDeptId eq '${this.tableDataFormCopy.SheetEventDeptId}'`
        );
      if (this.tableDataFormCopy.SheetEventClassOid)
        filter.push(
          `SheetEventClassOid eq '${this.tableDataFormCopy.SheetEventClassOid}'`
        );
      if (this.tableDataFormCopy.SheetEventDate) {
        if (this.tableDataFormCopy.SheetEventDate[0]) {
          filter.push(
            `SheetEventFromDate ge '${this.tableDataFormCopy.SheetEventDate[0]}'`
          );
        }
        if (this.tableDataFormCopy.SheetEventDate[1]) {
          filter.push(
            `SheetEventFromDate le '${this.tableDataFormCopy.SheetEventDate[1]}'`
          );
        }
      }
      filter = filter.length ? filter.join(" and ") : null;
      ...
      this.tableData = await this.$api.getSheetEvent(data);
    ...
    async del(x) {
      ...
      this.$store.commit("loading", true);
      await this.$api.delSheetEvent(x.SheetEventOid);
      await this.getData();
      this.$store.commit("loading", false);
    ...
    async defaultLabel(e) {
      let temp = "";
      if (e) {
        let data = {
          $filter: `SheetIndexOid eq '${e}'`,
        };
        temp = await this.$api.getAllPatient(data).then((res) => res.Items[0]);
      }
      this.defaultLabelName = `病歷號：${temp.ChartNo} / 姓名：${temp.PatientName} / 身分證字號：${temp.IdNo}`;
    },
    async sub() {
      this.$refs.postform.validate(async (valid) => {
        if (valid) {
          this.$store.commit("loading", true);
          if (this.isAdd) await this.$api.postSheetEvent(this.formCopy);
          else await this.$api.putSheetEvent(this.formCopy);
          await this.getData();
