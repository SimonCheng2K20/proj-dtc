    ...
    const menu = reactive({
      ...
      items: {
        one: [
          ...
          { label: "HIS歷史資料", key: "hIShistoricaldata" },
          {
            label: "健保署<br />雲端藥歷",
            key: "healthInsuranceDepartmentCloudMedicineRecord",
          },
          {
            label: "電子病歷<br />交換中心",
            key: "electronicMedicalRecordExchangeCenter",
          },
          { label: "調閱PACS", key: "accessPACS" },
        ],
        two: [
          { label: "視訊諮詢", key: "videoConsultation" },
          { label: "會診記錄", key: "historyMaintain" },
        ],
      },
    });

    function btnClickEvent(button) {
      emit("change", button.key);
    }

    return { menu, btnClickEvent };
  },
};
</script>
