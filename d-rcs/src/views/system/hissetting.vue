<template>
  <section class="dtc-main-section px-3">
    <section class="search-block">
      ...
      <div class="p-2 search-section">
        ...
        <div class="p-inputgroup">
          <span class="p-inputgroup-addon">主機名稱</span>
          <InputText type="text" v-model.trim="selectedName" @keydown.enter="getData" />
        </div>
        <div class="p-inputgroup">
          <span class="p-inputgroup-addon">主機API網址</span>
          <InputText type="text" v-model.trim="selectedUrl" @keydown.enter="getData" />
        </div>
        <button
    ...
    >
      ...
      <div class="content" :title="item.InstitutionName">{{ item.InstitutionName || '' }}{{ item.InstitutionNo ? `【${item.InstitutionNo}】` : '' }}</div>
      <div class="content" :title="item.HisName">
        {{ item.HisName || '' }}
      </div>
      <div class="content" :title="item.HisUrl">
        {{ item.HisUrl || '' }}
      </div>
      <div class="content" :title="item.UserName">
        {{ item.UserName || '' }}
      </div>
      <div class="content" :title="item.Permit">
        {{ item.Permit || '' }}
      </div>
      <div class="content" :title="item.TimeLimit">
        {{ item.TimeLimit || '' }}
      </div>
      <div class="content" :title="item.Method">
        {{ item.Method || '' }}
      </div>
    </main>
    ...
    <Dialog v-model:visible="editModal" style="width: 800px" :draggable="false" :modal="true" class="custom-modal">
      <template #header>
        <h3>
          {{ nowType == 1 ? '新增HIS主機' : nowType == 2 ? '編輯HIS主機' : '刪除HIS主機' }}
        </h3>
      </template>
      <section>
        <h2 v-if="nowType == 3" class="mb-2 font-black text-xl">是否確定要刪除HIS主機?</h2>
        <div class="p-inputgroup">
        ...
        <div class="p-inputgroup mt-2">
          <span class="p-inputgroup-addon">主機名稱</span>
          <InputText type="text" v-model.trim="modalItem.HisName" :disabled="nowType == 3" />
        </div>
        <div class="p-inputgroup mt-2">
          <span class="p-inputgroup-addon red-star">主機API網址</span>
          <InputText type="text" v-model.trim="modalItem.HisUrl" :disabled="nowType == 3" />
        </div>
        <div class="p-inputgroup mt-2">
          <span class="p-inputgroup-addon">使用者帳號</span>
          <InputText type="text" v-model.trim="modalItem.UserName" :disabled="nowType == 3" />
        </div>
        <div class="p-inputgroup mt-2">
          <span class="p-inputgroup-addon">使用者密碼</span>
          <InputText type="text" v-model.trim="modalItem.Permit" :disabled="nowType == 3" />
        </div>
        <div class="p-inputgroup mt-2">
          <span class="p-inputgroup-addon">資料撈取期限(月)</span>
          <InputText type="number" v-model.trim="modalItem.TimeLimit" :disabled="nowType == 3" :min="0" />
        </div>
        <div class="p-inputgroup mt-2">
          <span class="p-inputgroup-addon">資料撈取方式</span>
          <Dropdown v-model="modalItem.Method" :options="methodList" :disabled="nowType == 3" />
        </div>
      </section>
...
import { getHisList, putHisList, postHisList, deleteHisList, getInstitutionNos, getHisSettingMethodList } from 'Service/apis.js'
...
export default defineComponent({
  ...
  setup() {
    ...
    const headers = ref([
      ...
      { name: '主機名稱', key: 'HisName', sortDesc: null },
      { name: '主機API網址', key: 'HisUrl', sortDesc: null },
      { name: '使用者帳號', key: 'UserName', sortDesc: null },
      { name: '使用者密碼', key: 'Permit', sortDesc: null },
      { name: '資料撈取期限(月)', key: 'TimeLimit', sortDesc: null },
      { name: '資料撈取方式', key: 'Method', sortDesc: null },
    ])
    ...
    async function getData() {
      try {
        ...
        if (!(orderByArr.value.includes('InstitutionNo') || orderByArr.value.includes('InstitutionNo desc'))) {
        ...
        if (selectedName.value) {
          if (bQs) {
            qs = appendSubstringofFilter('HisName', qs, selectedName.value.toLowerCase())
          } else {
            qs = substringofFilter('HisName', qs, selectedName.value.toLowerCase())
          }
          bQs = true
        }
        ...
        if (selectedUrl.value) {
          if (bQs) {
            qs = appendSubstringofFilter('HisUrl', qs, selectedUrl.value.toLowerCase())
          } else {
            qs = substringofFilter('HisUrl', qs, selectedUrl.value.toLowerCase())
          }
          bQs = true
        }

        //top:筆數、skip:跳過幾筆
        const res = await getHisList(`${qs}`)
    ...
    async function saveEditModal() {
      if (!Boolean(modalItem.value.InstitutionNo) || !Boolean(modalItem.value.HisUrl)) {
        toast.error(`主機機構和主機API網址為必填欄位`, {
          timeout: 5000,
          hideProgressBar: true,
        })
        return
      ...
      try {
        ...
        if (nowType.value == 1) {
          const res1 = await postHisList(obj)
        }
        if (nowType.value == 2) {
          const res2 = await putHisList(obj)
        }
        if (nowType.value == 3) {
          // const id = `InstitutionNo(${modalItem.value.InstitutionNo}),HisUrl(${modalItem.value.HisUrl})`;
          const res3 = await deleteHisList(modalItem.value.Id)
        }
    ...
    const GetHisSettingMethodList = async () => {
      try {
        const res = await getHisSettingMethodList()
        if (res.status === 200) {
...
.dtc-grid {
  display: grid;
  grid-template-columns: 244px 400px 220px 330px 210px 1fr 150px 150px;
  text-align: center;
