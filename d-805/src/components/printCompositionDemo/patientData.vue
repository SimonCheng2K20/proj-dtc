...
export default {
  ...
  created () {
    this.fetchPatientSource();
  },
  async fetchPatientSource () {
    if (this.patientSourceList.length === 0) {
      const { actual: { Items } } = await getOptionsPatientSource()
    ...
    this.baseData.forEach((item) => {
      if (this.patientInfo[item.key]) {
        if (item.key === "ReportDateTime") {
          this.patientInfo[item.key] = this.patientInfo[item.key].substr(0, 8) + ' ' + this.patientInfo[item.key].substr(8)
        }
        item.value = item.key.includes('Date') || item.key.includes('Time') || item.key === 'ProcedureCompleted'
          ? this.$moment(this.patientInfo[item.key]).format('YYYY-MM-DD HH:mm')
          : item.key === "PatientSourceTypeCode"
            ? this.patientSourceList.find(({ value }) => value === this.patientInfo[item.key])['text']
            : this.patientInfo[item.key];
        if (item.key === 'ReportLastModifyTime' && this.patientInfo['ReportLastModifyTime'] == this.patientInfo['ReportCreateTime']) {
          item.value = '';
        }
      }
    });
