<template>
  <div class="page-inner">
    <h1
      class="py-2 mr-0 mt-0 ml-0 mb-0 report-title"
      v-html="report.ReportName"
    />
    <patientData :patientInfo="patientInfo" />
    <extraData
      :summyStr="report.Summary"
      :reportContentStr="report.ReportContent"
    />
...
export default {
  ...
  computed: {
    ...
    editerInfo () {
      if (this.report.IssuePhysicianName == this.report.ReportDoctor) return '';
      return `輸入者: ${this.report.ReportDoctor} 
        ${this.report.ReporterStaffType != "28"
          ? "醫師"
          : this.report.ReporterSex === "O" ? "小姐" : "先生"}`;
    },
    doctorInfo () {
      // return this.report.IssuePhysicianName != this.report.ReportDoctor ? `${this.report.IssuePhysicianName} 醫師` : "";
      return `${this.report.IssuePhysicianName} 醫師`;
    },
    // doctorInfo () {
