...
<template>
  <div id="twpv2">
    <b-row no-gutters class="mt-1">
      <b-col xl="12">
        <b-table
          ...
          @row-selected="(e)=>{enterReport ? SeletedRowFun(e) : NoSeletedRowFun()}"
          :sticky-header="heightForScroll"
          :no-local-sorting="noLocalSorting"
          :unifiedPerPage="unifiedPerPage"
        >
          <!-- Table BUSY state -->
          ...
          <template v-for="(s, i) in specialColumn" v-slot:[`cell(specialColumn${s.pos})`]="row">
            <div :key="'specialColumn' + i" v-if="s.type === 'hoverText'" v-text="s.text" @mousemove="s.event" @mouseleave="s.event" />

            <!-- procedureComplete 在1小時內 -->
            <img
              :key="'specialColumn' + i"
              v-if="
                s.type === 'image' &&
                row.item.EmgFlag === 'Y' &&
                ((row.item.Modality === 'CR' && $moment().diff(new Date(row.item.ProcedureCompleted)) < 1000 * 60 * 60) || // CR 1小時之內
                  ((row.item.Modality === 'MR' || row.item.Modality === 'CT') && $moment().diff(new Date(row.item.ProcedureCompleted)) < 1000 * 60 * 60 * 3)) // CT, MR 3小時之內
              "
              :src="s.img"
              style="width: 30px"
            />
            ...
            <!-- <div
              :key="'specialColumn' + i"
              v-if="s.type === 'image'"
              v-text="row.item.Modality"
            /> -->
          </template>

          <template v-slot:cell(PfcComment)="row">
            <div v-b-tooltip.hover :title="row.item.PfcComment" style="text-overflow: ellipsis; white-space: nowrap; overflow: hidden; max-width: 500px; text-align: left">{{ row.item.PfcComment }}</div>
          </template>

          <template v-slot:cell(Reason)="row">
            <div v-b-tooltip.hover :title="row.item.Reason" style="text-overflow: ellipsis; white-space: nowrap; overflow: hidden; max-width: 500px; text-align: left">{{ row.item.Reason }}</div>
          </template>

          <template v-slot:cell(Message)="row">
            <div v-b-tooltip.hover :title="row.item.Message" style="width: 100%; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">{{ row.item.Message }}</div>
          </template>
          ...
          <template v-slot:cell(OpColumnType1)="row">
            <b-btn v-show="OpButton1Func && isShowEdit" size="sm" variant="success" class="no-wrap mr-1" @click="OnColumnButton1Click(row.item)">{{
              Text4OpColButton1
            }}</b-btn>

            <b-btn
              v-show="OpButton2Func"
              v-if="checkIfDisableOpButton2Func"
              :disabled="checkIfDisableOpButton2Func ? checkIfDisableOpButton2Func(row.item) : true"
              size="sm"
              variant="info"
              class="no-wrap mr-1"
              @click="OnColumnButton2Click(row.item)"
              >{{ checkFavoriteCases(row.item) ? '已追蹤' : Text4OpColButton2 }}</b-btn
            >

            <b-btn v-show="OpButton3Func" size="sm" variant="info" class="no-wrap" @click="OnColumnButton3Click(row.item)">{{ Text4OpColButton3 }}</b-btn>
          </template>

          <template v-slot:cell(OpColumnType2)="row">
            <b-btn
              v-show="OpButtonXFunc"
              :disabled="checkIfDisableOpButtonXFunc ? checkIfDisableOpButtonXFunc(row.item) : true"
              size="sm"
              variant="danger"
              class="no-wrap"
              @click="OnColumnButtonXClick(row.item)"
            >
              {{ `放棄撰打` }}
            </b-btn>
          </template>
          ...
          <template v-slot:cell(ProcedureNameList)="row">
            <!-- <span v-b-tooltip.hover :title="row.item.ProcedureNameList">{{ trunCatedString(row.item.ProcedureNameList, 26) }}</span> -->
            <div v-b-tooltip.hover :title="row.item.ProcedureNameList" style="width: 100%; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">{{ row.item.ProcedureNameList }}</div>
          </template>
          ...
          <template v-slot:head(CheckBoxColumnType1)="head">
            <b-form-checkbox v-if="IfShowHeaderCheckBox" size="lg" v-model="allChecked" :value="true" :unchecked-value="false" @change="OnAllCheckBoxChange()">
              {{ head.label }}
            </b-form-checkbox>
          </template>
          <template v-slot:cell(SmsStatusText)="row">
            <div v-b-tooltip.hover :title="row.item.SmsStatusText" style="width: 100%; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">{{ row.item.SmsStatusText }}</div>
          </template>
          <template v-slot:cell(ReturnCodeText)="row">
            <div v-b-tooltip.hover :title="row.item.ReturnCodeText" style="width: 100%; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;">{{ row.item.ReturnCodeText }}</div>
          </template>
        </b-table>
      </b-col>
    </b-row>
    <b-row no-gutters class="mb-1" v-if="PagingEnabled">
      <b-col sm="4">
        <div class="float-left my-1 px-1 w-100" v-if="isShowEdit">
      ...
      <b-col sm="4">
        <b-pagination v-model="PagingCurrentPage" @change="OnPageChange" :total-rows="RowCount" :per-page="PagingRowPerPage" align="center">
      ...
      <b-col sm="4">
        <div class="float-right my-1 pr-1">
          <label id="twp5-label" for="pagingPerPageOption" class="float-left my-1" v-html="GetPageOptionLabel"></label>
          <b-form-select
      ...
      <b-col :sm="multiFuncBarSm" class="px-1 mt-1 text-right">
        ...
        <b-button v-show="ifShowMultiFuncs && MultiSelectFunc3" variant="info" class="ml-2" @click="OnFunc3">
          <font-awesome-icon icon="check-double" />
          {{ selectFunc3String }}
        </b-button>
        <b-button v-show="ifShowMultiFuncs" class="bg-danger ml-2" @click="OnCancelAllSelects">取消所有勾選</b-button>
      </b-col>
    </b-row>
  </div>
</template>
...
const OpButton1FuncDefault = (e) => {
  console.log(`OpButton1FuncDefault: (no func set)`, e)
}
...
export default {
  ...
  props: {
    ...
    sortChangeFunc: { type: Function },
    selectFunc1String: { type: String, default: '編輯' },
    ...
    //判斷是否enable OpButton2Func
    checkIfDisableOpButton2Func: { type: Function },
    checkIfDisableOpButtonXFunc: { type: Function },
    isNeedSorting: { type: Boolean, default: true },
    noLocalSorting: { type: Boolean, default: false },
    initToPage: { type: Number, default: 1 },
    unifiedPerPage: { type: Boolean, default: false },
  },
  data() {
    return {
      Wait4AsyncLoad: false,
      PagingEnabled: true,
      PagingRowPerPageTemp: 10,
      PagingCurrentPage: 1,
      TableSortBy: '',
      TableSortDesc: false,
      Columns: [],
  ...
  computed: {
    ...
    ifShowMultiFuncs() {
      return (this.MultiSelectFunc1 || this.MultiSelectFunc2 || this.MultiSelectFunc3) && this.checkedItems.length > 0
    },
    CalcDataContent() {
      if (this.RowData) {
    ...
    countNum: {
      get(){
        return this.countNumTemp
      },
      set(val){
        this.countNumTemp = val
      } 
    },
    enterSeletData: {
      get(){
        return this.enterSeletDataTemp
      },
      set(val){
        this.enterSeletDataTemp = val
      }
    },
    PagingRowPerPage: {
      get(){
        return this.PagingRowPerPageTemp
      },
      set(val){
        this.PagingRowPerPageTemp = val
      }
    }
  },
  methods: {
    checkFavoriteCases(item){
      if(item && item.Status == '71' && item.IsFavoriteCases === true){
        return true
      }
      return false
    },
    ...
    InitUiParams() {
      ...
      if(!this.unifiedPerPage){
        if (strPerPage) {
          this.PagingRowPerPage = parseInt(strPerPage)
        }
      }else{
        let strPerPageAll = localStorage.getItem('all_perPage')

        if(strPerPageAll){
          this.$emit('unifiedPerPageFun', parseInt(strPerPageAll))
          this.PagingRowPerPage = parseInt(strPerPageAll)
        }       
      }
    },
    //
    ...
    setPerPage(){
      if(this.unifiedPerPage){
        this.$emit('unifiedPerPageFun', this.PagingRowPerPage)
      }
    },
    ChangePerPage() {
    ...
    OnColumnButton1Click(x) {
      ...
      if (this.ifSendBackTheList) {
        ...
        if (this.isNeedSorting) {
          copy.sort((m, n) => {
            return m.AccessionNo == x.AccessionNo ? -1 : n.AccessionNo == x.AccessionNo ? 1 : 0
          })
        }
        this.OpButton1Func(copy, this.TableId, x)
      } else {
    ...
    OnColumnButtonXClick(x) {
      this.OpButtonXFunc(x)
    },
    ...
    async AsyncFetch() {
      ...
      try{
        ...
        console.log(`AsyncFetch: `, this.TableId, this.toPage, this.PagingRowPerPage)

        let dataSet = await this.DataFetch(this.TableId, this.toPage, this.PagingRowPerPage)
            ...
            if (this.Rows && this.Rows.length) {
              this.Rows.forEach((i) => {
                ...
                this.checkedItems.forEach((j) => {
                  if (i.OrderNo && j.OrderNo && i.OrderNo === j.OrderNo) i.TWPCheckBox1Value = true
                })
              })
            }
          }
    ...
    ONSortChanged() {
      if (this.sortChangeFunc)
        setTimeout(
          () =>
            this.sortChangeFunc({
              by: this.TableSortBy,
              desc: this.TableSortDesc,
              id: this.TableId,
            }),
          50
        )
    ...
    RemoveItemFromChecked(item) {
      this.checkedItems = this.checkedItems.filter((i) => i.AccessionNo !== item.AccessionNo)
    },
    AppendItemToChecked(item) {
      if (
        this.checkedItems.some((i) => {
          if (i.AccessionNo == item.AccessionNo) return true
        })
      )
        return
    ...
    IfShowCheckBox(item) {
      if (this.CheckIfShowRowCheckBox) {
        item.showCheckBox = this.CheckIfShowRowCheckBox(item)
    ...
    TabWatch(e){
      if(e.key === 'Enter' && this.enterSeletData && e.target.nodeName === 'TR'){
        let realData = this.enterSeletData[0]
        this.OnColumnButton1Click(realData)
        this.enterSeletData = null
      }
    },
    NoSeletedRowFun(){
      return false 
    ...
    SeletedRowFun(data){      
      if(data && data.length){
        this.enterSeletData = data
      }
    },
  },
  created() {
    this.toPage = this.initToPage
  ...
  mounted() {
    ...
    window.addEventListener('keyup', this.TabWatch)
    ...
    if (this.Operatable || this.OpButton1Func) {
      ...
      if (this.OpButton3Func) {
        this.Text4OpColButton3 = !this.OpButton3Text ? '管理' : this.OpButton3Text
      }
    }
    //
    if (this.CheckBox1Func && !this.CheckBox1Lead) {
    ...
    this.Columns = this.CheckBox1Lead ? fields : fields.filter(({ key }) => key !== 'CheckBoxColumnType1')
  ...
  destroyed(){
    window.removeEventListener('keyup', this.TabWatch)
  },
  watch: {
    async RowCount(val) {
      if (val && this.$route.query.page) {
        await this.$nextTick()
        this.PagingCurrentPage = Number(this.$route.query.page)
      }
    },
  }
}
