<template>
  <!-- 為了要改變背景放mounted-css -->
  <div id="FI0309" class="pageContent pb-1 mounted">
      ...
      <b-card-header class="bg-warning text-dark cardHeader btn-right">
        ...
        <b-row no-gutters>
          <b-col cols="6" class="float-left text-left font-weight-bold"
            >個案追蹤 用戶: [{{ $store.state.Profile4User.staffName }}]
      ...
      <b-card-text class="text-left py-1 m-0">
        <b-container fluid class="px-0">
          ...
          <b-row no-gutters class="mt-1" v-show="showDetails">
            <b-col sm="2" class="px-1">
              <b-card bg-variant="transparent" class="border" no-body>
            ...
            <b-col sm="4" class="px-1">
              <b-card bg-variant="transparent" class="border" no-body>
                <b-card-header class="bg-danger text-white font-weight-bold cardHeader">
                  <b-row no-gutters>
                    <b-col cols="6" class="float-left text-left">臨床申請檢查註記</b-col>
                  </b-row>
                </b-card-header>
                <b-card-text class="p-0">
                  <b-container fluid class="p-0">
          ...
          <b-row no-gutters class="mt-1 mx-0">
                ...
                >
                  <b-form :id="item.TagId">
                    <b-row>
                      ...
                      ></b-col>
                      <b-col v-else id="blkReportInput" sm="12" class="px-1">
                        <b-textarea
                          id="DxRMain"
                          no-resize
                          class="border-dark"
                          ...
                          placeholder
                          style="height: 500px; border-style: none"
                        />
                        <!-- <b-textarea id="DxRExtra" no-resize rows="4" class="border-warning mt-1" hidden ref="reportExtra" v-model="DxReportExtraNote" placeholder="" /> -->
                      </b-col>
                    </b-row>
                  </b-form>
                </b-tab>
            ...
            <b-col v-if="showcst" id="blkReportInput" sm="8" class="px-1">
              <b-textarea
                id="DxRMain"
                no-resize
            ...
            <b-col sm="3" class="px-1">
              <b-card no-body bg-variant="transparent" class="border h-100">
                <b-card-header class="bg-success font-weight-bold cardHeader">
                  <b-row no-gutters>
                    <b-col class="float-left text-left text-white">個案備註</b-col>
                  </b-row>
                </b-card-header>
                <b-card-text class="px-1 pb-1 sb4RHis" style="width: 100%; height: 100%">
                    ...
                    <b-row class="m-0">
                      <b-col cols="12" class="pl-1 button-align">
                        ...
                        <b-button
                          variant="primary"
                          class="mx-1 mt-1"
                          :to="{ name: 'FI0308', params: tempFI0308UiConditions }"
                          replace
                          >返回</b-button
                        >
                        ...
                        <b-button variant="primary" class="mx-1 mt-1" @click="callPacs"
                          >PACS</b-button
                        >
                        <b-button
                          variant="danger"
                  ...
                  <b-textarea
                    ...
                    no-resize
                    v-model="favoriteCaseComment"
                  ></b-textarea>
                  <div class="px-2" style="color: #666666">
                    目前字數:
                    {{
                      favoriteCaseComment && favoriteCaseComment.length
                        ? favoriteCaseComment.length
                        : '0'
                    }}/1000
                  </div>
                  <!-- <b-textarea v-else readonly no-resize style="height:calc(100% - 345px)" v-model="evaluation.DxRAEComment" placeholder="科內審查意見"></b-textarea> -->
                  <!-- </div> -->
                </b-card-text>
              </b-card>
            </b-col>
    ...
    <b-modal ref="evaluationSaveAlert" header-bg-variant="danger" hide-footer title="儲存提醒">
      ...
      <b-col class="text-right px-1 mt-1">
        <b-button class="mr-1" md="3" offset-md="3" variant="danger" @click="saveEvaluate"
          >確認</b-button
        >
    ...
    >
      ...
      ></b-form-radio-group>
      <b-textarea
        style="height: calc(100vh - 320px); color: white"
        no-resize
        rows="2"
...
export default {
  ...
  data() {
    return {
      ...
      favoriteCaseComment: null,
      //暫存FI0308(前頁) 搜尋conditions
      tempFI0308UiConditions: null,
    };
  },
  computed: {
    // state() {
    ...
    numObj2() {
      return {
        visibility:
          this.currentNum >= this.totalArrayLen || this.hideNextPrevBtn ? 'hidden' : 'visible',
        cursor: 'pointer',
      };
    },
    urgentPlaceholder() {
  ...
  methods: {
        ...
        callPaceWebSocket(url)
          .then((res)=>{
            if(res){
              ...
              this.ObjApp.TimingMessage('呼叫PACS成功');
            }
          }).catch((err)=>{
    ...
    async saveEvaluate() {
      ...
      this.$router.replace({ path: '/MI03/FI0308' });
    },
    //儲存 個案追蹤 備註
    async saveFavoriteCaseComment() {
      // 啟動 報告存檔 作業
      // console.log(this.$logs("modify my favorite case comment:" + status));
      // 報告存檔 - 後台存取
      this.ObjApp.WaitingShow();
      //開始儲存
      let opStatus = false;
      let opResult = '存取時發生錯誤!';
      let subPath = '/api/FavoriteCases';
      this.favoriteCaseData.PfcComment = this.favoriteCaseComment;
      let dataBody = this.favoriteCaseData;
      await window.axios
    ...
    get999Status() {
      const ok1 =
        this.theReport.DxreportPhysicianId === this.loginId && this.theReport.DxrStatus == 71;
    ...
    async mapUIParamFromData() {
      if (this.theReport) {
        let d = this.theReport;
      ...
      await new DataManager({
        ...
        .executeQuery(
          new Query()
            .where('AccessionNo', 'equal', this.theReport.AccessionNo)
            .sortBy('ProcedureCompleted')
        )
        .then((r) => {
          itemPack = r.actual.Items; //資料;
    ...
    async viewRecordByCurrentNumber() {
      let passedData = window.favoriteData;
      let accessionNo = passedData.AccessionNo;
      let version = passedData.DxrVersion;
      this.favoriteCaseData = passedData;
      this.favoriteCaseComment = passedData.PfcComment;
      ...
      this.theReport = await window.axios.get(
        '/Report/Get?accessionNo=' + accessionNo + '&version=' + version
      );
    ...
    async viewNextRecord(redirect) {
      ...
      if (this.currentNum > this.totalArrayLen) {
        --this.currentNum;
        if (redirect) this.$router.replace({ name: 'FI0308' });
        return;
      }
      ...
      this.$nextTick(() => {
        ...
        if (valueSet && valueSet.length > 0) {
          valueSet.forEach((e) => {
            ...
            let item = $(`#${customizeIdLead}${idx} [name="${e.name}"]`);
            //
            // console.log(item[0]);
            if (item[0] && (item[0].type === 'radio' || item[0].type === 'checkbox')) {
              $.each(item, (index, content) => {
                if (content.value === e.value) {
                  content.checked = true;
                  return false;
    ...
    async DataGethering1() {
      ...
      console.log(this.favoriteCaseData.DxrVersion);
      let url = this.favoriteCaseData.AccessionNo;
      if (this.favoriteCaseData.DxrVersion) {
        url += '&version=' + this.favoriteCaseData.DxrVersion;
      }
      this.theReport = await window.axios.get('/Report/Get?accessionNo=' + url);

      //對應UI 參數
      this.mapUIParamFromData();
      //指定目前版本
      console.log(this.theReport);
      ...
      let basicReport = {
        ...
        StructureImage: null,
        StructureData: null,
        StructureContent: this.theReport.ReportContent,
        TagId: `${customizeIdLead}${0}`,
      };
      this.cstTab.push(basicReport);
      //
      this.cstreport = '';
      if (cstdata.length > 0) {
        // 有癌篩報告
        this.cstreport = false;
        this.cstTab = this.cstTab.concat(cstdata);
      }
      //
  ...
  async mounted() {
    ...
    if (this.$route.params) {
      this.tempFI0308UiConditions = this.$route.params;
    }
