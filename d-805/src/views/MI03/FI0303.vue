          ...
          <b-col>
            <div class="float-left ml-1 py-1">
              <b-btn
                ...
                size="sm"
                @click="Edit4Append"
              ><i class="fas fa-plus"></i> 新增資料</b-btn>
            </div>
          </b-col>
        </b-row>
        <!--
      ...
      if (fieldRef) {
        setTimeout(x => {
          this.$nextTick(() => {
            ...
            //為解決CKEditor在modal裡的 popup input控制項無法編輯的問題,
            //把 dom裡面的tabindex屬性移除 便可
            let modal = $(`#WinEdit0303___BV_modal_content_`);
            if (modal) modal.removeAttr("tabindex");
          });
        }, 500);
      }
    },
